package com.example.gps_g11.Controller.NaoVaiSerPreciso;
...
public class ExpenseTabController {
    ...
    public void AddExpense() {
        if(Name.getText().isEmpty() || Value.getText().isEmpty() || CategoriaCheckbox.getValue().toString().equals("Empty") || DatePicker.getValue().toString().isEmpty())
           ...
           /* boolean result = context.addExpense(CategoriaCheckbox.getValue().toString(), Descripton.getText(), DatePicker.getValue(), Float.parseFloat(Value.getText()), Recurring.isSelected());
            if(result){
